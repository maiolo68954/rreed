...
install_vnc_server(){
  ...
  if [ "$ARCH" = "x86_64" ]; then
    ...
    echo -e "${RED}[ERR] Unsupported architecture: $ARCH${RESET}"
    return 1
  fi

  VGL_URL="https://github.com/VirtualGL/virtualgl/releases/download/${VGL_VER}/virtualgl_${VGL_VER}_${ARCH}.deb"
  wget -q "$VGL_URL" -O /tmp/virtualgl.deb || {
    ...
    # Try alternative download locations or build from source
    with_sudo apt-get install -y virtualgl || {
      echo -e "${RED}[ERR] Could not install VirtualGL${RESET}"
      return 1
    }
  }
  
  if [ -f "/tmp/virtualgl.deb" ]; then
    with_sudo dpkg -i /tmp/virtualgl.deb || with_sudo apt-get -y -f install
  fi

  echo -e "${CYAN}[*] Downloading & Installing TurboVNC...${RESET}"
  TURBO_VER="3.1.1"
  TURBO_URL="https://github.com/TurboVNC/turbovnc/releases/download/${TURBO_VER}/turbovnc_${TURBO_VER}_${ARCH}.deb"
  wget -q "$TURBO_URL" -O /tmp/turbovnc.deb || {
    echo -e "${RED}[ERR] Failed to download TurboVNC${RESET}"
    echo -e "${YELLOW}Installing standard TigerVNC instead...${RESET}"
    with_sudo apt-get install -y tigervnc-standalone-server tigervnc-xorg-extension || {
      echo -e "${RED}[ERR] Could not install VNC server${RESET}"
      return 1
    }
    # Set VNC_BIN for TigerVNC
    VNC_BIN="vncserver"
  }
  ...
  echo -e "${CYAN}[*] Configuring VirtualGL (if installed)...${RESET}"
  if [ -x "/opt/VirtualGL/bin/vglserver_config" ]; then
    with_sudo /opt/VirtualGL/bin/vglserver_config -config +s +f -t </dev/null
  else
    echo -e "${YELLOW}[WARN] VirtualGL not found, skipping configuration${RESET}"
  fi
...
EOF
  ...
  if [ -x "/opt/TurboVNC/bin/vncpasswd" ]; then
  ...
  if [ -x "$VNC_BIN" ]; then
    ...
    $VNC_BIN :1 -geometry 1920x1080 -depth 24
  else
    echo -e "${RED}[ERR] No VNC server binary found${RESET}"
    return 1
  fi
...
show_ip_ports(){
  ...
  # Check for TurboVNC first, then TigerVNC
  if [ -x "/opt/TurboVNC/bin/vncserver" ] && /opt/TurboVNC/bin/vncserver -list >/tmp/vnclist 2>/dev/null; then
  ...
  elif command -v vncserver >/dev/null 2>&1 && vncserver -list >/tmp/vnclist 2>/dev/null; then
    echo -e "${GRAY}Detected displays:${RESET}"
    grep -oE ":[0-9]+" /tmp/vnclist | sort -u | while read -r disp; do
      num="${disp#:}"; port=$((5900 + num))
      echo -e "  • display ${BOLD}:${num}${RESET} → port ${BOLD}${port}${RESET} (connect: ${IP}:${port})"
...
stop_vnc_server(){
  ...
  if [ -x "/opt/TurboVNC/bin/vncserver" ]; then
    ...
    echo -e "${YELLOW}No VNC server found.${RESET}"
    return 0
  fi
